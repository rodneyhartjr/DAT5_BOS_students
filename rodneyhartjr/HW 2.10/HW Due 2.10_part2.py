# -*- coding: utf-8 -*-
"""
Created on Thu Feb 12 01:32:18 2015
...
# SQL
sql = """select playerID, yearID, sum(R) as total_runs, sum(H) as total_hits, sum(SB) as stolen_bases, sum(SO) as strikeouts, sum(IBB) as total_intentional_walks
from Batting
where yearID > 1954
and yearid < 2005 
group by playerID
...
### looks like some heteroskedasticity for higher hit totals
#
## let's calculate RMSE -- notice you use two multiply signs for exponenets
## in Python
RMSE = (((df.residuals) ** 2).mean() ** (1/2))
## so, on average, the model is of by about 37 runs for each observation.
#
...
## looks like in-sample deviation is 33.8% on average. 
#
#
## now, let's see the effects of stolen_bases on total_runs -- do we think that,
...
## is it better or worse than using hits? Worse
#
...
#
## is RMSE higher or lower for stolen bases? Higher
print RMSE_sb
print RMSE
#
## lets understand the percent by which the model deviates from actuals on average
...
## looks like in-sample deviation is 154.6% on average -- higher than the 33.8% of 
## hits.
#
